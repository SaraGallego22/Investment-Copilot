...
def test_bad_api_key_raises_a_clear_error(market):
    from collaborative_partner.tools import market_tool
    ...
    try:
        with TestClient(app) as bad:
            ...
            with pytest.raises(market_tool.MarketUnavailable, match="403"):
                market_tool.get_market_snapshot("crash", 0)
    finally:
...
def test_selling_now_vs_holding_reports_both_and_a_caveat():
    ...
    result = selling_now_vs_holding({"TECHX": 0.5, "UTILCO": 0.5}, "recovery", 20)
...
def test_empty_portfolio_is_rejected():
    ...
    with pytest.raises(ValueError):
        project_portfolio({"TECHX": 0.0}, "crash", 10)
